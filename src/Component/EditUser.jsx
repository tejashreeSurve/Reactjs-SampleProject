...
class EditUser extends Component {
    ...
    showUser() {
        ...
        getUser(userid, userToken).then(response => {
            console.log(response.data.message.fName);
            this.setState({
                fName: response.data.message.fName,
                mName: response.data.message.mName,
                lName: response.data.message.lName,
                userEmail: response.data.message.userEmail,
                password: response.data.message.password,
                isVerified: response.data.message.isVerified
            })
        }
        )
    }
    ...
    updateUser() {
        if (this.validateForm()) {
            ...
            editUser(this.state.userId, user, token).then(response => {
        ...
        return <div>
            <Card className="updateForm">
                <CardContent>
                    <h4>Update Form</h4>
                    <div>
                        <TextField variant="outlined" margin="normal" required fullWidth id="userId" label="user Id"
                            name="userId" value={this.state.userId} onChange={this.axios} disabled
                        ></TextField>

                        <TextField variant="outlined" margin="normal" required fullWidth id="firstname" label="First Name"
                            name="fName" value={this.state.fName} onChange={this.axios}
                        ...
                        <TextField variant="outlined" margin="normal" required fullWidth="true" id="middlename" label="Middle Name"
                            name="mName" value={this.state.mName} onChange={this.axios}
                        ...
                        <TextField variant="outlined" margin="normal" required fullWidth="true" id="lastname" label="Last Name"
                            name="lName" value={this.state.lName} onChange={this.axios}
                        ...
                        <TextField variant="outlined" margin="normal" required fullWidth="true" id="email" label="Email Address"
                            name="userEmail" value={this.state.userEmail} onChange={this.axios} disabled
                        ></TextField>


                        <TextField variant="outlined" margin="normal" required fullWidth="true" id="email" label="Password"
                            name="password" value={this.state.password} type="password" onChange={this.axios} disabled
                        ></TextField>

                        <Button className="signup" type="submit" fullWidth variant="contained" color="primary" onClick={() => this.updateUser()}>Update</Button>
                    </div>
                </CardContent>
            </Card>
        </div>
